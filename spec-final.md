...
---
**Problem statement (1 câu):** *Phụ huynh & học sinh VinSchool mất 10–15 phút/lần tra thủ công qua PDF Cẩm nang hoặc chờ hotline để hỏi điểm, lịch thi, học phí — AI Agent kết hợp RAG cá nhân hóa + dữ liệu cộng đồng trả lời tức thì < 5 giây, đúng theo từng học sinh.*
...
Quality — Precision over Recall: Với dữ liệu tài chính và học tập của trẻ em, sai 1 con số nguy hiểm hơn là im lặng. Target: Hallucination ≤ 3%, Answer Correctness ≥ 90%, Citation Rate ≥ 95%. Đo bằng RAGAS framework automated + human eval 200 câu/sprint.

Risk chính — 3 cái cần giải ngay từ đầu: (1) Dữ liệu stale không báo cho user → fix bằng timestamp bắt buộc + TTL cache ngắn. (2) Tài liệu năm cũ trả lời tự tin → fix bằng academic_year filter trong mọi RAG query. (3) Nhầm học sinh khi phụ huynh có nhiều con → fix bằng server-side mapping enforcement, không tin client.

Data flywheel: Mỗi lần user bấm "Phản hồi không đúng" → log {query, response, error_type, chunk_id} → weekly review → cập nhật RAG index hoặc sửa tool prompt → accuracy tăng dần. CSAT và Hotline Deflection Rate là 2 north star metric đo hiệu quả thực tế sau mỗi tháng. Không có model fine-tuning trong v1 — toàn bộ cải thiện qua prompt + RAG data quality.
